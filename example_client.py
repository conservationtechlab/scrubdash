...
I simulate this by having asyncio run update_img_src(), which reads hardcoded image paths from a csv and updates a file variable. To ensure that this path will work with dash, I also run a dash server that updates the image source to the path contained in the file variable. Since the file variable is changing every 2 seconds from update_img_src(), dash should be rerendering the image every 2 seconds as well (dash rerenders every 2 seconds since I set the dcc.Interval component to update n_intervals every 2 seconds).
"""
